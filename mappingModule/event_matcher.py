...
def publish_matched_event(result, rabbitmq_host="localhost", exchange="disaster_updates", routing_key="update"):
    try:
        ...
        connection.close()
        # Зберігаємо результат у Redis
        redis_client.set("latest_matched_events", message)
    except Exception as e:
        logging.error("Failed to publish matched event: %s", e)


def match_event_to_disasters(user_event, distance_threshold=500000):
    ...
    event_coords = geocode_location(user_event)
    if not event_coords:
